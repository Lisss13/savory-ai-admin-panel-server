...
async def test_list_returns_envelope_with_camel_case(
    ...
    data = body["data"]
    assert data["page"] == 1 and data["pageSize"] == 20 and data["total"] == 1
    item = data["items"][0]
...
async def test_list_pagination(
    ...
    """`pageSize=1` отдаёт ровно одну запись и сохраняет реальный `total`."""
    org = await make_organization()
    for _ in range(3):
        await make_subscription(organization_id=org.id)

    resp = await client.get("/api/v1/subscriptions?page=2&pageSize=1", headers=auth_headers)
