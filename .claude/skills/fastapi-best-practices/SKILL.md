...
description: >
  Apply FastAPI + Pydantic v2 + SQLAlchemy 2.0 async best practices when writing,
  reviewing, or modifying code in this repository. Use whenever the task touches
  FastAPI routes, Pydantic models, SQLAlchemy ORM or async sessions, Alembic
  migrations, JWT auth, BackgroundTasks, dependency injection, or pytest+httpx
  tests — even if the user doesn't name FastAPI explicitly. Also use on diff
  review to catch the listed anti-patterns.
---

# FastAPI Best Practices for AI Agents

A machine-readable companion to README.md for AI coding agents
...
Pin to these versions or newer. Examples assume them.
...
`app/api/routes/`, `app/models/`, `app/schemas/`. When adding a new bounded
context, prefer the domain-package layout above.

**Cross-domain imports** — always use the explicit module name. Never `from app.auth import *`.
...
| Importing across domains via deep paths (`from app.auth.service.user import ...`) | Tight coupling, hard to refactor. | `from app.auth import service as auth_service`. |
